...
LCU gameflow의 큐와 게임 모드를 식별해 랭크 게임에서는 기존 포지션별 픽/밴을 수행하고, 칼바람에서는 포지션 기반 작업을 생략한 채 챔피언 선택부터 인게임·종료 감시까지 정상적으로 이어갑니다. 그 밖의 모드는 챔피언 선택을 자동 조작하지 않고 안전하게 감시만 수행합니다.
...
- 랭크·칼바람 게임 모드 인식과 모드별 챔피언 선택 처리
...
로비에서 대전 찾기를 수동으로 취소하면 현재 자동화 실행도 정상 종료되며 다시 큐를 시작하지 않습니다. LoL
